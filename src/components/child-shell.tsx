...
export function ChildShell({ children, hideNavigation = false }: { children: React.ReactNode; hideNavigation?: boolean }) {
  ...
  return <div className="min-h-dvh bg-pitch-50 text-ink">
    ...
    <main className={`mx-auto max-w-lg px-4 ${hideNavigation ? "pb-8" : "pb-28"}`}>{children}</main>
    {!hideNavigation && <nav className="fixed inset-x-0 bottom-0 z-20 mx-auto flex max-w-lg justify-around border-t border-pitch-100 bg-white/95 px-2 pb-[max(.6rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur" aria-label="Aðalvalmynd">
